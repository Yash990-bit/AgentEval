...
class AgentTrustEdge(Base):
    ...
    source_agent_id = Column(UUID(as_uuid=True), nullable=False, index=True)
    target_agent_id = Column(UUID(as_uuid=True), nullable=False, index=True)
    ...
    # JSONB array of objects {"tick": int, "score": float}
